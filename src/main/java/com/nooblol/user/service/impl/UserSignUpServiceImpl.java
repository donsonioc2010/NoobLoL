...
public class UserSignUpServiceImpl implements UserSignUpService {
    ...
    public boolean sendSignUpUserMail(UserSignUpRequestDto userDto) {
        return userSendMailService.sendMail(userDto.getUserEmail(), getAuthMailContent(userDto));
    }
    ...
    private void isNotUnAuthUser(UserSignUpRequestDto user) {
        if (!UserRoleStatus.isUserUnAuthUser(user.getUserRole())) {
            throw new IllegalArgumentException(user.getUserName() + "님의 계정은 활성화가 필요한 상태가 아닙니다.");
        }
    }
}

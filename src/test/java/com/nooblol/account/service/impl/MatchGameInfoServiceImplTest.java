...
class MatchGameInfoServiceImplTest {
    ...
    @InjectMocks private MatchGameInfoServiceImpl matchGameInfoService;

    @Mock private MatchGameInfoMapper matchGameInfoMapper;

    @Mock private MatchGameAddInfoMapper matchGameAddInfoMapper;

    @Mock private RiotConfiguration riotConfiguration;

    @Mock private MatchGameListService matchGameListService;

    String responseNotFoundPuuid = "abcdefgh";
    String responseOkPuuid =
    ...
    void confirm_getMakeUri() {
        ...
        when(riotConfiguration.getMatchGameInfoByMatchId())
                .thenReturn("lol/match/v5/matches/{matchId}");
    ...
    void getMatchSimpleListReturnByDBTest() throws Exception {
        ...
        when(matchGameAddInfoMapper.selectMatchSimpleList(any()))
                .thenReturn((ArrayList<MatchGameSimpleDto>) mockReturnList);

        List<MatchGameSimpleDto> returnList =
                (List<MatchGameSimpleDto>)
                        matchGameInfoService.getMatchInfoListByPuuid(responseOkPuuid, 0, 30).getResult();

...
class AdminControllerTest {
        ...
        mockMvc
                .perform(
                        RestDocumentationRequestBuilders.delete(
                                        "/admin/forceUserDelete/{deleteUserId}", requestUserId)
                                .session(adminSession))
                .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                .andExpect(jsonPath("$.result", Is.is(true)))
                .andDo(
                        document(
                                ...
                                pathParameters(parameterWithName("deleteUserId").description("강제 삭제하고자 하는 사용자 ID")),
                                DocumentSnippetsUtils.responseHeaders_ContentTypeApplicationJsonValue(),
                                DocumentSnippetsUtils.responseFields_IsOkStatusAndResultTrue()));
    }
    ...
    void getAllUserList_WhenIsRequestUserIsAdmin_ThenReturnUserList() throws Exception {
        ...
        // mock
        when(adminService.getAllUserList(pageNum * limitNum, limitNum)).thenReturn(mockReturnDto);

        // when & then
        mockMvc
                .perform(
                        RestDocumentationRequestBuilders.get("/admin/userList")
        ...
        mockMvc
                .perform(
                        RestDocumentationRequestBuilders.put(
                                        "/admin/userChangeToActive/{changeUserId}", requestUserId)
                                .session(adminSession))
                .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                .andExpect(jsonPath("$.result", Is.is(true)))
                .andDo(
    ...
    void changeToSuspensionUser_WhenIsRequestUserIsAdmin_ThenReturnTrue() throws Exception {
        ...
        // mock
        when(adminService.changeToSuspensionUser(requestUserId))
                .thenReturn(ResponseFixtureUtils.getResultCodeIsOkAndResultTrueFixture());
        ...
        mockMvc
                .perform(
                        RestDocumentationRequestBuilders.put(
                                        "/admin/userChangeToSuspension/{changeUserId}", requestUserId)
                                .session(adminSession))
                .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                .andExpect(jsonPath("$.result", Is.is(true)))
                .andDo(

...
class UserControllerTest {
    ...
    @MockBean UserSignOutService userSignOutService;
                ...
                .andDo(
                        document(
                                ...
                                requestFields(
                                        ...
                                        fieldWithPath("orgUserName")
                                                .type(JsonFieldType.STRING)
                                                .description("현재 사용중인 사용자 이름"),
                                        fieldWithPath("newUserName")
                                                .type(JsonFieldType.STRING)
                                                .description("변경하고자 하는 사용자 이름"),
                                        fieldWithPath("orgPassword")
                                                .type(JsonFieldType.STRING)
                                        ...
                                        fieldWithPath("reqUserInfoUpdateRoleValidation")
                                                .description("사용자 이름 또는 사용자 명에 대한 검증으로, 둘중 한개는 입력이 되어야 한다."),
                                        fieldWithPath("updatedAt").ignored()),
                                DocumentSnippetsUtils.responseHeaders_ContentTypeApplicationJsonValue(),
                                DocumentSnippetsUtils.responseFields_IsOkStatusAndResultTrue()));
    }
        ...
        mockMvc
                ...
                .andDo(
                        document(
                                ...
                                DocumentSnippetsUtils.requestHeaders_ContentTypeApplicationJsonValue(),
                                requestFields(
                                        fieldWithPath("userId").type(JsonFieldType.STRING).description("삭제하려는 사용자 ID"),
    ...
    void userLogin_WhenIsExistsEmailAndPassword_ThenReturnUserInfoAndOk() throws Exception {
        ...
        // mock
        when(userInfoService.userLogin(any(), any())).thenReturn(mockResponseDto);
                        ...
                        document(
                                ...
                                responseFields(
                                        ...
                                        fieldWithPath("result.userName")
                                                .type(JsonFieldType.STRING)
                                                .description("사용자 이름"),
                                        fieldWithPath("result.userRole")
                                                .type(JsonFieldType.NUMBER)
                                                .description("계정 권한"),
                                        fieldWithPath("result.level")
                                                .type(JsonFieldType.NUMBER)
                                                .description("계정 커뮤니티 레벨"),
                                        fieldWithPath("result.exp")
                                                .type(JsonFieldType.NUMBER)
    ...
    void userLogout_WhenIsSuccess_ThenReturnTrueAndOk() throws Exception {
        ...
        // when & then
        mockMvc
                .perform(RestDocumentationRequestBuilders.post("/user/logout").session(session))
                .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                .andExpect(jsonPath("$.result", Is.is(true)))
                .andDo(
                        document(
    ...
    void resendAuthMail_WhenIsSuccess_ThenReturnTrueAndStatusOk() throws Exception {
        ...
        // mock
        when(userSignUpService.reSendSignUpUserMail(requestEmail.trim()))
                .thenReturn(ResponseFixtureUtils.getResultCodeIsOkAndResultTrueFixture());
        ...
        mockMvc
                ...
                .andDo(
                        document(
                                "user/resendAuthMail",
                                pathParameters(parameterWithName("email").description("재수신을 받고자 하는 사용자 이메일")),
                                DocumentSnippetsUtils.responseHeaders_ContentTypeApplicationJsonValue(),
                                DocumentSnippetsUtils.responseFields_IsOkStatusAndResultTrue()));
    }
    ...
    void authUserByMail_WhenIsSuccess_ThenReturnTrueAndStatusOk() throws Exception {
        ...
        // mock
        when(userSignUpService.changeRoleAuthUser(requestUserId))
                .thenReturn(ResponseFixtureUtils.getResultCodeIsOkAndResultTrueFixture());

        // when & then
        mockMvc
                .perform(RestDocumentationRequestBuilders.get("/user/auth/{userId}", requestUserId))
                .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                .andExpect(jsonPath("$.result", Is.is(true)))
                .andDo(
                        document(

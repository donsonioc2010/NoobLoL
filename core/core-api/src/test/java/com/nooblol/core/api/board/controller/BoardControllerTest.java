...
class BoardControllerTest {
    ...
    class CategoryTest {
                    ...
                    .andDo(
                            document(
                                    ...
                                    responseFields(
                                            ...
                                            fieldWithPath("result[].categoryId")
                                                    .type(JsonFieldType.NUMBER)
                                                    .description("카테고리ID"),
                                            fieldWithPath("result[].categoryName")
                                                    .type(JsonFieldType.STRING)
                                                    .description("카테고리 명"),
                                            fieldWithPath("result[].status")
                                                    .type(JsonFieldType.STRING)
                                                    .description("해당 카테고리 사용방법 상태"),
                                            fieldWithPath("result[].createdUserId")
                                                    .type(JsonFieldType.STRING)
                                                    .description("생성한 관리자 ID"),
                                            fieldWithPath("result[].createdAt")
                                                    .type(JsonFieldType.STRING)
                                                    .description("생성일"),
                                            fieldWithPath("result[].updatedUserId")
                                                    .type(JsonFieldType.STRING)
        ...
        void insertCategory_WhenInsertSuccess_ThenReturnOkAndTrue() throws Exception {
            ...
            CategoryInsertDto requestDto =
                    new CategoryInsertDto()
                            .builder()
                            .categoryName("Sample Add Category")
                            .status(CategoryStatus.ACTIVE)
                            .createdAt(LocalDateTime.now())
                            .updatedAt(LocalDateTime.now())
                            .build();
            ...
            // mock
            when(categoryService.insertCategory(any(CategoryInsertDto.class), any(HttpSession.class)))
                    .thenReturn(true);
        ...
        void updateCategory_WhenUpdateSuccess_ThenReturnOkAndTrue() throws Exception {
            // given
            int categoryId = 1;
            CategoryUpdateDto requestDto =
                    new CategoryUpdateDto()
                            .builder()
                            .categoryId(categoryId)
                            .newCategoryName("Sample Update CategoryName")
                            .status(CategoryStatus.ACTIVE)
                            .build();
            ...
            // mock
            when(categoryService.updateCategory(any(CategoryUpdateDto.class), any(HttpSession.class)))
                    .thenReturn(true);

            // when & then
            mockMvc
                    .perform(
                            RestDocumentationRequestBuilders.put("/board/category")
                                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                                    .content(objectMapper.writeValueAsBytes(requestDto))
                                    .session(session))
                    .andExpect(status().isOk())
                    ...
                    .andDo(
                            document(
                                    ...
                                    requestFields(
                                            ...
                                            fieldWithPath("newCategoryInfoValid")
                                                    .type(boolean.class)
                                                    .description(
            ...
            mockMvc
                    .perform(
                            RestDocumentationRequestBuilders.delete(
                                            "/board/category/{categoryId}", requestCategoryId)
                                    .session(session))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                    .andExpect(jsonPath("$.result", Is.is(true)))
    ...
    class BbsTest {
        ...
        void getBbsList_WhenGivenCategoryId_ThenReturnOkAndBbsList() throws Exception {
            ...
            List<BbsDto> responseList =
                    BoardFixtureUtils.BbsListFixtureByCategoryIdAndStatusAndUserID(
        ...
        void getAllBbsList_WhenRequest_ThenReturnOkAndAllBbsList() throws Exception {
            ...
            // when & then
            mockMvc
                    .perform(RestDocumentationRequestBuilders.get("/board/bbsAllList"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.resultCode", Is.is(HttpStatus.OK.value())))
                    .andDo(
                            document(
                                    "board/bbs/getAllList",
                                    DocumentSnippetsUtils.responseHeaders_ContentTypeApplicationJsonValue(),
                                    boardResponseFields()));
        }
        ...
        void updateBbs_WhenUpdateSuccess_ThenReturnOkAndTrue() throws Exception {
            ...
            mockMvc
                    .perform(
                            RestDocumentationRequestBuilders.put("/board/bbs")
                                    .contentType(MediaType.APPLICATION_JSON_VALUE)
                                    .content(objectMapper.writeValueAsBytes(requestDto))
                                    .session(session))
                    .andExpect(status().isOk())
